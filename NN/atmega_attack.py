...
from constants import CNN_MODEL_PATH_ATMEGA, MLP_MODEL_PATH_ATMEGA, MLP_PRE_MODEL_PATH_ATMEGA, PATH_TRACES_ATMEGA, BYTE_IDX, DELTA, ATMEGA_TEST_SIZE, SBOX_ATMEGA,ATMEGA_TRAIN_SIZE
...
def main():
    ...
    tot_traces = ATMEGA_TEST_SIZE

    if arch == "CNN_ZAID":
        MODEL_ROOT = f'{CNN_MODEL_PATH_ATMEGA}TRAINING/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
        RESULTS_ROOT = f'{CNN_MODEL_PATH_ATMEGA}ATTACK/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
    elif arch == "MLP":
        MODEL_ROOT = f'{MLP_MODEL_PATH_ATMEGA}TRAINING/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
        RESULTS_ROOT = f'{MLP_MODEL_PATH_ATMEGA}ATTACK/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
    elif arch == "MLP_PRE":
        MODEL_ROOT = f'{MLP_PRE_MODEL_PATH_ATMEGA}TRAINING/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
        RESULTS_ROOT = f'{MLP_PRE_MODEL_PATH_ATMEGA}ATTACK/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
    ...
    # ...
    test_files = [f'{PATH_TRACES_ATMEGA}']
    ...
    test_dl = TestDataLoader(
        test_files,
        tot_traces=tot_traces, # Qui usa ATMEGA_TEST_SIZE
        target=target,
        byte_idx=b,
        out_mode=out_mode,
        delta=delta,
        op="SUM",
        sbox=SBOX_ATMEGA.flatten()
    )
