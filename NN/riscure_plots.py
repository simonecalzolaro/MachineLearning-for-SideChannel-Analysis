...
try:
    from constants import PATH_PLOTS_RISCURE_PINATA
except ImportError:
...
ARCHS = ["CNN_ZAID", "MLP"] # Rimosso MLP_PRE
...
# ==========================================
# 3. HELPER FUNCTIONS (RICERCA DINAMICA)
# ==========================================
def get_out_dir(arch, metric=""):
    d = os.path.join(PATH_PLOTS_RISCURE_PINATA, arch, metric) if metric else os.path.join(PATH_PLOTS_RISCURE_PINATA, arch)
...
if __name__ == "__main__":
    print("========================================")
    print("Generazione Plot RISCURE PIÑATA (CNN_ZAID e MLP)")
    print("========================================\n")
    ...
    for arch in ARCHS:
        ...
        plot_unified_bar(arch_models, "traces", "Minimum Traces (GE<0.5)", f"{arch} - Attack Efficiency (Riscure)", os.path.join(out_traces, f"{arch}_Traces_Bar.png"), '#377eb8')
        plot_ge_evolution_line(arch_models, f"Guessing Entropy Evolution - {arch} (Riscure)", os.path.join(out_traces, f"{arch}_GE_Evolution_Line.png"))
        ...
        plot_radar_chart(arch_models, f"5D Pareto Analysis: Top {arch} Models (Riscure)", os.path.join(out_pareto, f"{arch}_Radar_Pareto.png"))


    if best_overall_models:
        print("\nGenerazione Comparison dei Best Models (Riscure)...")
        ...
        plot_unified_bar(best_overall_models, "traces", "Minimum Traces (GE<0.5)", "BEST MODELS - Attack Efficiency (Riscure)", os.path.join(comp_dir, "TRACES", "All_Traces_Bar.png"), '#377eb8')
        plot_ge_evolution_line(best_overall_models, "BEST MODELS - Guessing Entropy Evolution (Riscure)", os.path.join(comp_dir, "TRACES", "All_GE_Evolution_Line.png"))
        ...
        plot_radar_chart(best_overall_models, "Ultimate 5D Pareto: Best Models Comparison (Riscure)", os.path.join(comp_dir, "PARETO", "Ultimate_Radar_Pareto.png"))

    print("\nTutto generato con successo per Riscure Piñata!")