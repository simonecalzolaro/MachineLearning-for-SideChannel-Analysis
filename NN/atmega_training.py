...
from constants import CNN_MODEL_PATH_ATMEGA, MLP_MODEL_PATH_ATMEGA, MLP_PRE_MODEL_PATH_ATMEGA, PATH_TRACES_ATMEGA, BYTE_IDX, DELTA, ATMEGA_TRAIN_SIZE, SBOX_ATMEGA
...
def main():
    ...
    tot_traces = ATMEGA_TRAIN_SIZE

    if arch == "CNN_ZAID":
        RES_ROOT = f'{CNN_MODEL_PATH_ATMEGA}TRAINING/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
    elif arch == "MLP":
        RES_ROOT = f'{MLP_MODEL_PATH_ATMEGA}TRAINING/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
    elif arch == "MLP_PRE":
        RES_ROOT = f'{MLP_PRE_MODEL_PATH_ATMEGA}TRAINING/{target}/{dataset}_key_{out_mode}_out_{ptx_mode}_ptx'
    ...
    train_files = [f'{PATH_TRACES_ATMEGA}']
    ...
    train_dl = SplitDataLoader(
        ...
        delta=delta,
        op="SUM",
        sbox=SBOX_ATMEGA.flatten() # <--- Passa S-Box
    )
